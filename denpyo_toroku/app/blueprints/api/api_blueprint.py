...
_DEFAULT_SELECT_AI_MODEL_ID = "xai.grok-4.3"
...
# 分類用サンプル分析は常に 1 回だけ実行する。
_CATEGORY_ANALYSIS_ATTEMPT_COUNT = 1
...
def _runtime_oci_defaults() -> Dict[str, str]:
    ...
    return {
        ...
        "llm_model_id": _normalize_text(
            ...
            _normalize_text(getattr(AppConfig, "LLM_MODEL_ID", ""), "xai.grok-4.3"),
        ),
...
def _save_oci_settings(settings_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    settings_for_env = {
        ...
        "llm_model_id": _normalize_text(
            ...
            current_settings.get("llm_model_id", "xai.grok-4.3"),
        )
        or "xai.grok-4.3",
        "vlm_model_id": _normalize_text(
...
def _build_oci_model_test_settings(request_settings: Dict[str, Any]) -> Dict[str, str]:
    ...
    llm_model_id = _normalize_text(
        ...
        snapshot_settings.get("llm_model_id", defaults.get("llm_model_id", "xai.grok-4.3")),
    ) or "xai.grok-4.3"
    vlm_model_id = _normalize_text(
