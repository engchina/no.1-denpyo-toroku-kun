...
- v0.0.3: 大阪リージョンのみをサポートしています。（デフォルト：大阪リージョン）

  Click [![Deploy to Oracle Cloud](https://oci-resourcemanager-plugin.plugins.oci.oraclecloud.com/latest/deploy-to-oracle-cloud.svg)](https://cloud.oracle.com/resourcemanager/stacks/create?region=ap-osaka-1&zipUrl=https://github.com/engchina/no.1-denpyo-toroku-kun/releases/download/v0.0.3/v0.0.3.zip)
...
└── denpyo_toroku/               # メインアプリケーション
    ...
    ├── css/                      # スタイルシート・画像
    ├── js/
    │   ├── main.js               # 旧 JET エントリ (互換用)
    │   ├── root.js               # 旧 Knockout 初期化
    │   ├── appController.js      # 旧メインコントローラー
    │   ├── views/                # 旧 HTML テンプレート
    │   └── viewModels/           # 旧 Knockout ViewModel
    ├── app/
...
このリポジトリには、旧版の意図分類モデル学習フローは含まれていません。
現在の業務フローでは、ローカルで `.pkl` モデルを学習して配置する必要はありません。

初回セットアップでは、以下を行ってください。

### 1. OCI / Object Storage / Database 接続設定

Web UI の設定画面、または対応 API で次を設定します。

- OCI 接続情報
- Object Storage 設定
- Database / Wallet 設定
- OCI GenAI / Select AI モデル設定
...
```bash
curl -s -X POST "http://localhost:8080/api/v1/database/init" \
  | python3 -m json.tool
```

### 3. 伝票分類とプロンプトの整備

業務投入前に、以下を UI から整備しておくのを推奨します。
...
- `curl http://localhost:8080/api/v1/health` が応答するか確認
- Gunicorn / Flask が起動しているか確認
- リバースプロキシやポート設定 (`8080` / `5000`) が環境と一致しているか確認

### OCI 認証エラー

```text
OCI authentication or model initialization failed
```
...
- `VLM_MODEL_ID` が正しいモデル ID (`google.gemini-2.5-pro`) であるか確認
